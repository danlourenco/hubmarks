...
export class SyncManager {
  ...
  private async executeSync(config: SyncConfig, startTime: number): Promise<SyncResult> {
    ...
    try {
      ...
      console.log('🔍 [SyncManager.executeSync] JSON client exists:', !!this.jsonClient);
      
      // Ensure JSON client is available
      if (!this.jsonClient) {
  ...
  private async getRemoteBookmarks(): Promise<StoredBookmark[]> {
    ...
    try {
      ...
      const file = await this.jsonClient.getFileContent('bookmarks.json');
  ...
  private async applyChanges(
    ...
    try {
      ...
      if (changes.toGitHub.length > 0 && this.jsonClient) {
        let existingBookmarks: StoredBookmark[] = [];
        let existingSha: string | undefined;
        
        try {
          // Always read current remote JSON file to get full bookmark set
          const existing = await this.jsonClient.getFileContent('bookmarks.json');
        ...
        if (existingSha) {
          ...
          await this.jsonClient.updateFile(
            'bookmarks.json',
            jsonContent,
          ...
          await this.jsonClient.createFile(
            'bookmarks.json',
            jsonContent,
        ...
        try {
          ...
          const existingReadme = await this.jsonClient.getFileContent('README.md');
          await this.jsonClient.updateFile(
            'README.md',
            markdown,
